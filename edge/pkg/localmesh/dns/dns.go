...
func startDNS() {
	...
	if err != nil {
		klog.Errorf("[LocalMesh] dns server listen on %v error: %v", laddr, err)
		return
	}
	defer udpConn.Close()
	dnsConn = udpConn
	for {
		...
		if err != nil || n <= 0 {
			klog.Errorf("[LocalMesh] dns server read from udp error: %v", err)
			continue
		}
		...
		if err != nil {
			klog.Warningf("[LocalMesh] failed to resolve dns: %v", err)
			continue
		}
...
func lookupFromMetaManager(serviceURL string) (exist bool, ip string) {
	...
	klog.Errorf("[LocalMesh] service %s is not found in this cluster", serviceURL)
	return false, ""
}

// getIPFromEP gets ips from endpoints
...
func getFromRealDNS(req []byte, from *net.UDPAddr) {
	...
	if err != nil {
		klog.Errorf("[LocalMesh] parse nameserver err: %v", err)
		return
	}
